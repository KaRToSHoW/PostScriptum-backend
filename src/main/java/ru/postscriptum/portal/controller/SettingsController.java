...
public class SettingsController {
    ...
    public ResponseEntity<?> get(Authentication auth) {
        ...
        Map<String, Object> row = jdbc.queryForMap(
            "SELECT notification_email, notification_push, notification_sms, reminder_hours_before, theme, interface_locale " +
            "FROM user_settings WHERE user_id = ?", me);

        return ResponseEntity.ok(toSettingsMap(row));
    }
    ...
    public ResponseEntity<?> update(Authentication auth, @RequestBody Map<String, Object> body) {
        ...
        Integer reminderH  = body.get("reminderHoursBefore") != null ? ((Number) body.get("reminderHoursBefore")).intValue() : 24;
        ...
        jdbc.update("""
            INSERT INTO user_settings (user_id, notification_email, notification_push, notification_sms,
                reminder_hours_before, theme, interface_locale, updated_at)
            VALUES (?,?,?,?,?,?,?,NOW())
            ON CONFLICT (user_id) DO UPDATE SET
                ...
                updated_at         = NOW()
            """, me, notifEmail, notifPush, notifSms, reminderH, theme, locale);
